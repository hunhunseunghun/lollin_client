...
const UserInfoGrade = ({ participant }) => {
  ...
  useEffect(() => {
    axios
      .get(`https://lollinserver.link/members/score?nickname=${participant.summonerName}`)
      .then((res) => {
      ...
      <section className="gradeWrap">
        <div className="gradeIconWrap">
          ...
          {/* <img
            ...
            className="gardeIconImg"
          /> */}
        <img src={GradeIcon({score : userGrade})} alt="" className="gardeIconImg" />
        </div>
        {!isVeryBad? <div className="gradeText">{GradeText}</div> : 
        <div className="gradeText VERYBAD">VERY BAD</div>}
        {!intGrade? <div className="point">평가 기록이 없습니다.</div> : <div className="point">{`${userGrade}`}</div>}
      </section>
    </Container>
  );
};
